...
$$ 
\log(\lambda) = \beta_0 + s_1(\text{elev}) + s_2(\text{forest}) + s_3(\text{hfi}) + s_4(\text{distwater}) 
$$
...
model_linear <- ppm(bee_ppp ~ elev + forest + hfi + distwater, covariates = covariates_list)
model_linear

model_quad <- ppm(bee_ppp ~ elev + I(elev^2) +
                             forest + I(forest^2) +
                             hfi + I(hfi^2) +
                             distwater + I(distwater^2),
...
anova(model_linear, model_quad, test = "Chi")
```
...
```{r, cache=TRUE}
intensity_im <- predict(model_quad, type = "trend", n = 200)
log_intensity_im <- log(intensity_im)

plot(log_intensity_im,
     se = FALSE,
     superimpose = FALSE,
     main = "Log Fitted Intensity (Quadratic Model)")
...
elev_effect <- effectfun(model_quad, "elev", 
                         forest = med_forest,
                         hfi = med_hfi,
                         distwater = med_dist,
                         se.fit = FALSE)

forest_effect <- effectfun(model_quad, "forest", 
                           elev = med_elev,
                           hfi = med_hfi,
                           distwater = med_dist,
                           se.fit = FALSE)

hfi_effect <- effectfun(model_quad, "hfi", 
                        elev = med_elev,
                        forest = med_forest,
                        distwater = med_dist,
                        se.fit = FALSE)

dist_effect <- effectfun(model_quad, "distwater", 
                         elev = med_elev,
                         forest = med_forest,
                         hfi = med_hfi,
                         se.fit = FALSE)
...
Quadratic Model Did not Converge. So we can show how the linear model performed too

```{r, cache=TRUE}
intensity_im <- predict(model_linear, type = "trend", n = 200)
log_intensity_im <- log(intensity_im)
...
elev_effect <- effectfun(model_linear, "elev", 
                         forest = med_forest,
                         hfi = med_hfi,
                         distwater = med_dist,
                         se.fit = TRUE)

forest_effect <- effectfun(model_linear, "forest", 
                           elev = med_elev,
                           hfi = med_hfi,
                           distwater = med_dist,
                           se.fit = TRUE)

hfi_effect <- effectfun(model_linear, "hfi", 
                        elev = med_elev,
                        forest = med_forest,
                        distwater = med_dist,
                        se.fit = TRUE)

dist_effect <- effectfun(model_linear, "distwater", 
                         elev = med_elev,
                         forest = med_forest,
                         hfi = med_hfi,
                         se.fit = TRUE)
...
```{r}
quadrat.test(model_quad, nx = 5, ny = 5)
```

```{r}
res <- residuals(model_quad)
print(res)
plot(res, cols = "transparent")
```

```{r, cache=TRUE}
par_res_elev <- parres(model_quad, covariate = "elev")
par_res_forest <- parres(model_quad, covariate = "forest")
par_res_hfi <- parres(model_quad, covariate = "hfi")
par_res_dist <- parres(model_quad, covariate = "distwater")
